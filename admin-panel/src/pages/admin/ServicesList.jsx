...
import { Plus, Edit2, Trash2, ToggleLeft, ToggleRight, Wrench, Search } from 'lucide-react';
...
export default function ServicesList() {
  ...
  const filteredServices = services.filter((s) => {
    ...
    return (
      ...
      (s.nameUr || '').toLowerCase().includes(q) ||
      (s.descriptionEn || '').toLowerCase().includes(q) ||
      (s.descriptionUr || '').toLowerCase().includes(q) ||
    ...
    <div className="animate-fade-in">
      ...
      {filteredServices.length === 0 ? (
        <div className="card-premium p-16 flex flex-col items-center justify-center">
          ...
          <p className="text-sm text-ink-600">{search ? 'No matching services' : 'No services yet'}</p>
        </div>
      ) : (
