  ...
  return (
    <div className="py-10 animate-fade-in max-w-4xl mx-auto">
      <div className="text-center mb-8">
        <div className="inline-flex items-center justify-center w-14 h-14 rounded-2xl bg-gradient-to-br from-brand-600 to-brand-800 text-white mb-4 shadow-lg shadow-brand-600/20">
          <Sparkles size={24} />
        </div>
      ...
      <form onSubmit={handleSearch} className="flex gap-3">
        <div className="relative flex-1">
          ...
          <input
            ...
            className="w-full pl-11 pr-4 py-4 bg-white border border-gray-200 rounded-2xl focus:outline-none focus:ring-2 focus:ring-brand-500 shadow-sm text-sm"
          />
        </div>
        <button
          type="submit"
          disabled={loading || !query.trim()}
          className="inline-flex items-center gap-2 px-6 py-4 bg-brand-600 hover:bg-brand-700 text-white font-semibold rounded-2xl transition-colors disabled:opacity-50"
        >
      ...
      <div className="mt-5 grid grid-cols-1 sm:grid-cols-2 gap-2">
        {examples.map((ex) => (
          <button
            key={ex}
            onClick={() => setQuery(ex)}
            className="text-left px-4 py-3 bg-white border border-gray-100 rounded-xl text-sm text-gray-500 hover:border-brand-300 hover:text-brand-700 transition-colors"
          >
      ...
      {error && (
        <div className="mt-6 p-4 rounded-2xl bg-red-50 text-red-700 text-sm">{error}</div>
      )}
      ...
      {parsed && !loading && (
        <div className="mt-8">
          <div className="mb-6 p-5 rounded-2xl bg-gradient-to-br from-brand-50 to-emerald-50 border border-brand-100">
            <div className="flex items-center gap-2 mb-3">
              <Sparkles size={15} className="text-brand-600" />
              <p className="text-sm font-semibold text-brand-800">AI Understanding</p>
            </div>
            <div className="flex flex-wrap gap-3">
              {parsed.service && (
                <span className="inline-flex items-center gap-1.5 px-3 py-1.5 rounded-full bg-white border border-brand-200 text-xs font-medium text-brand-700">
              ...
              {parsed.urgency && (
                <span className="inline-flex items-center gap-1.5 px-3 py-1.5 rounded-full bg-white border border-amber-200 text-xs font-medium text-amber-700">
