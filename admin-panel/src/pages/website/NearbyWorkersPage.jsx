...
import { MAPBOX_TOKEN, MAPBOX_STYLE, DEFAULT_CENTER, DEFAULT_ZOOM, getBrowserLocation } from '../../lib/mapbox';
import { getStoredLocation, calculateDistance } from '../../lib/location';
import { MapPin, Locate, Loader2, ChevronDown, ChevronLeft, SlidersHorizontal } from 'lucide-react';
...
export default function NearbyWorkersPage() {
  ...
  const [center, setCenter] = useState(stored || DEFAULT_CENTER);
  ...
  return (
    <div className="py-4 md:py-8 animate-fade-in pb-24 md:pb-8">
      {/* Mobile header */}
      <div className="md:hidden px-4 mb-4 flex items-center justify-between">
        ...
        <div className="text-center">
          <h1 className="font-display font-bold text-ink-900">Nearby Workers</h1>
          <p className="text-[10px] text-gray-500">{workers.length} verified professionals near you</p>
        </div>
      ...
      <div className="px-4 md:px-0 md:max-w-7xl md:mx-auto">
        {/* Desktop header */}
        <div className="hidden md:flex md:flex-row md:items-center justify-between gap-4 mb-7">
          <div>
        ...
        {/* Mobile radius + locate */}
        <div className="md:hidden flex items-center gap-2 mb-4">
          <div className="relative flex-1">
            <select
              value={radius}
              onChange={(e) => handleRadiusChange(Number(e.target.value))}
              className="w-full pl-3 pr-8 py-2.5 text-sm bg-white border border-gray-200 rounded-xl focus:outline-none focus:ring-2 focus:ring-brand-500 appearance-none cursor-pointer"
            >
          ...
          <button
            onClick={locate}
            disabled={locating}
            className="btn-primary inline-flex items-center gap-1.5 px-4 py-2.5 text-white text-sm font-semibold rounded-xl disabled:opacity-50"
          >
            {locating ? <Loader2 className="animate-spin" size={14} /> : <Locate size={14} />}
            Locate
          </button>
        </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-4 md:gap-6">
            ...
            {loading ? (
              <div className="grid grid-cols-1 gap-4 md:gap-5">
                {Array.from({ length: 4 }).map((_, i) => (
                  <div key={i} className="h-28 md:h-72 bg-gray-100 rounded-2xl md:rounded-3xl animate-pulse" />
                ))}
              </div>
            ) : workers.length === 0 ? (
              <div className="text-center py-16 md:py-20 bg-white rounded-2xl md:rounded-3xl border border-gray-100 shadow-card">
                <div className="w-14 h-14 md:w-16 md:h-16 mx-auto rounded-2xl bg-brand-50 flex items-center justify-center mb-4">
                  <MapPin className="text-brand-600" size={28} />
                </div>
                <h3 className="font-display font-bold text-gray-700">No workers nearby</h3>
                <p className="text-xs md:text-sm text-gray-400 mt-1">Try increasing the radius or using your current location.</p>
              </div>
            ) : (
          ...
          <div className="lg:col-span-1 order-first lg:order-last">
            <div className="lg:sticky lg:top-20 rounded-2xl md:rounded-3xl overflow-hidden border border-gray-100 shadow-card h-56 md:h-[480px]">
