...
const menuGroups = [
  ...
  },
  {
    label: 'Jobs',
    items: [
      { path: '/worker/requests', icon: CalendarClock, label: 'Booking Requests' },
  ...
  },
  {
    ...
    items: [
      ...
      { path: '/worker/verification', icon: ShieldCheck, label: 'CNIC Verification' },
      { path: '/worker/portfolio', icon: FolderOpen, label: 'Portfolio' },
      { path: '/worker/services', icon: Wrench, label: 'Services' },
      { path: '/worker/areas', icon: MapPin, label: 'Service Areas' },
      { path: '/worker/hours', icon: Clock, label: 'Working Hours' },
      { path: '/worker/payments', icon: CreditCard, label: 'Payment Methods' },
      { path: '/worker/disputes', icon: AlertTriangle, label: 'Disputes' },
    ...
    <div className="flex h-screen bg-[#F6F9F7]">
      ...
      <aside className={`fixed lg:static inset-y-0 left-0 z-30 w-72 bg-gradient-to-b from-brand-800 via-brand-700 to-brand-900 shadow-2xl transform transition-all duration-300 lg:translate-x-0 flex flex-col ${
        ...
        <div className="shrink-0 p-4 border-t border-white/10 bg-brand-800/50 backdrop-blur-sm">
          ...
          >
            <Briefcase size={18} /> View Website
          </button>
          <button
            ...
            <div className="relative" ref={userMenuRef}>
              ...
              {userMenuOpen && (
                <div className="absolute right-0 mt-2 w-56 bg-white rounded-2xl shadow-xl border border-gray-100 py-2">
                  ...
                  <button
                    onClick={() => { setUserMenuOpen(false); navigate('/worker/profile'); }}
                  ...
                  <button
                    ...
                    className="flex items-center gap-3 w-full px-4 py-2.5 text-sm text-gray-700 hover:bg-gray-50"
                  >
                    <Briefcase size={16} /> View Website
                  </button>
                  <button
                    onClick={handleLogout}
