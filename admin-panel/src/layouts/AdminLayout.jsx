...
import {
  LayoutDashboard, Users, Briefcase, Calendar, FolderTree, Wrench,
  AlertTriangle, Image, Bell, LogOut, Menu, X, TrendingUp, FileText,
  Activity, Shield, Settings, MessageSquare, ChevronDown, Search,
...
const menuGroups = [
  ...
  },
  {
    ...
    items: [
      ...
      { path: '/workers', icon: Briefcase, label: 'Workers' },
      { path: '/workers/pending', icon: UserCircle, label: 'Verification' },
  ...
  },
  {
    ...
    items: [
      ...
      { path: '/support-tickets', icon: MessageSquare, label: 'Support' },
...
export default function AdminLayout() {
  ...
  const isActive = (path) => {
    if (path === '/workers/pending') return location.pathname === '/workers/pending';
    if (path === '/workers') return location.pathname === '/workers';
    return location.pathname.startsWith(path) && location.pathname !== '/workers/pending';
  };
